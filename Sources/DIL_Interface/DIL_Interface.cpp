...
static DILI_UDS_Protocol* sg_pouDILI_UDS = NULL;  //No deberia estar en NULL
...
if sg_pouFP_CAN is null */
        ...
        break;
		case UDS:				//ADDED by Alejandra Sanchez
        {
            if (NULL == sg_pouDILI_UDS)
            {
                if ((sg_pouDILI_UDS = new DILI_UDS_Protocol ) == NULL)
                {
                    ASSERT(FALSE);
                    hResult = S_FALSE;
                }
                ...
                {
                    sg_pouDILI_UDS->InitInstance();
                }
            }
            // Else the object has been existing already
            *ppvInterface = (void*) sg_pouDILI_UDS; 
        }
        break;
